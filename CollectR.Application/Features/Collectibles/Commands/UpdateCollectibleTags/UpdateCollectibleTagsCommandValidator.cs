...
public sealed class UpdateCollectibleTagsCommandValidator
    : AbstractValidator<UpdateCollectibleTagsCommand>
{
    public UpdateCollectibleTagsCommandValidator()
    {
        RuleFor(x => x.Id).NotEmpty().WithMessage("Collectible Id is required.");

        RuleFor(x => x.TagIds)
            .NotNull()
            .WithMessage("TagIds collection is required.")
